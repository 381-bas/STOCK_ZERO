...
def main():
    ...
    if modo == "CLIENTE":
        ...
        def _render_cliente_exports() -> None:
            if scope_level not in {"L1", "L2", "L3"}:
                return
            ...
            detail_rename = {
                ...
                "cliente": "CLIENTE",
                "responsable_tipo": "RESPONSABLE TIPO",
                "responsable": "RESPONSABLE",
                "rutero": "RUTERO",
                "reponedor": "REPONEDOR",
                "marca": "MARCA",
            ...
            if scope_level == "L1":
                st.markdown("#### Descargas cliente")
                st.caption("Descargas globales del cliente. No consideran filtros opcionales de marca, foco o búsqueda.")

                try:
                    with _timed("EXPORT cliente_inventario_global_query", tag="CACHE"):
                        df_cliente_raw = db.get_detalle_sku_scope_total_page(
                            marca=None,
                            cliente=cliente_sel,
                            responsable_tipo=responsable_tipo_sel,
                            responsable=responsable_sel,
                    ...
                    df_cliente_input = _drop_total_rows(df_cliente_raw).rename(columns=detail_rename)
                except Exception as e:
                    _dbg("FAIL cliente_L1_exports", err=repr(e))
                    st.warning("No pude preparar las descargas globales del cliente.")
                    return

                if df_cliente_input is None or df_cliente_input.empty:
                    st.caption("Sin filas exportables para el cliente seleccionado.")
                    return

                df_inventory_cliente = build_inventory_cliente_export_df(df_cliente_input)
                df_focus_cliente = build_focus_export_df(df_cliente_input, foco=None)

                ex1, ex2 = st.columns(2, gap="small")
                with ex1:
                    inventory_excel = export_excel_generic(
                        f"CLIENTE_{scope_token}",
                        df_inventory_cliente,
                    )
                    st.download_button(
                        "Inventario cliente",
                        data=inventory_excel,
                        file_name=f"STOCK_ZERO_CLIENTE_INVENTARIO_{scope_token}_{file_stamp}.xlsx",
                ...
                with ex2:
                    focus_excel = export_excel_generic(
                        f"CLIENTE_{scope_token}_FOCO",
                        df_focus_cliente,
                    )
                    st.download_button(
                        "Foco Cliente",
                        data=focus_excel,
                        file_name=f"STOCK_ZERO_CLIENTE_FOCO_{scope_token}_{file_stamp}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
            ...
            with ex2:
                with st.expander("EXPORTAR SALAS FOCOS", expanded=False):
                    try:
                        ...
                        df_focus_input = _drop_total_rows(df_focus_raw).rename(columns=detail_rename)
                    except Exception as e:
                    ...
                    if df_focus_input is None or df_focus_input.empty:
                        ...
                        if selected_mercaderista:
                            df_focus_input = df_focus_input[
                                df_focus_input["REPONEDOR"].apply(lambda v: _match_multi_value(v, selected_mercaderista))
                            ].copy()
