...
def main():
    ...
    if modo == "CLIENTE":
        ...
        scope_label = "KPI" if kpi_only_tipo_scope else scope_level
        ...
        if kpi_only_tipo_scope:
            if responsable_tipo_all:
                st.caption("RESPONSABLE TIPO = Todos: carga resumida solo KPI.")
            else:
                st.caption("Selecciona GESTOR o SUPERVISOR para habilitar panel global y lectura táctica.")
            st.stop()
        ...
        def _render_cliente_exports() -> None:
            ...
            with ex1:
                with st.expander("EXPORTAR LOCALES", expanded=False):
                    try:
                        ...
                        st.warning("No pude preparar la exportación de locales del scope.")
                        df_local_export = pd.DataFrame()

                    if df_local_export is None or df_local_export.empty:
                        st.caption("Sin filas de locales para exportar.")
                    else:
                        local_excel = export_excel_generic(f"CLIENTE_{scope_token}", df_local_export)
                        local_pdf = export_pdf_generic(title_base + ["GESTIÓN REPOSICIÓN · Scope local"], df_local_export, list(df_local_export.columns))
                        st.download_button(
                            "Descargar Excel",
                            data=local_excel,
            ...
            with ex2:
                with st.expander("EXPORTAR FOCOS DEL SCOPE", expanded=False):
                    try:
                    ...
                    if df_focus_export is None or df_focus_export.empty:
                        ...
                        focus_pdf = export_pdf_focus_table(title_base + ["GESTIÓN DE INDICADORES · Scope"], df_focus_export)
                        st.download_button(
                            "Descargar Excel",
                            data=focus_excel,
