...
def main():
    ...
    if modo == "CLIENTE":
        ...
        with top2:
            st.selectbox(
                ...
                on_change=_reset_on_cliente_marca_change,
            )

        marca_sel = _scope_value_or_none(
        ...
        try:
            with _timed("SELECTOR clientes_home_scope", tag="SELECTOR"):
                clientes_scope = db.get_clientes_home_scope(marca=marca_sel)
                _dbg("CLIENTE clientes loaded", rows=len(clientes_scope), marca=marca_sel or "Todas")
                _dbg_block()
        except Exception as e:
        ...
        row2c1, row2c2 = st.columns([1.2, 2.0], gap="small")

        with row2c1:
            st.selectbox(
                "RESPONSABLE TIPO",
                [RESP_TIPO_PLACEHOLDER, "GESTOR", "SUPERVISOR"],
                key="sel_responsable_tipo",
                on_change=_reset_on_responsable_tipo_change,
            )

        responsable_tipo_sel = _scope_value_or_none(
            st.session_state.get("sel_responsable_tipo"),
            RESP_TIPO_PLACEHOLDER,
        )
        ...
        with row2c2:
            st.selectbox(
                ...
                disabled=not bool(responsable_tipo_sel),
            )
        ...
        if marca_sel and not clientes_scope:
            st.warning("La marca seleccionada no tiene clientes operativos en scope.")
        ...
        meta1, meta2, meta3, meta4, meta5 = st.columns([1.2, 1.4, 1.2, 1.4, 1.0], gap="small")
        meta1.caption(f"Scope: {scope_level}")
        meta2.caption(f"Marca: {marca_sel or 'Todas'}")
        meta3.caption(f"Cliente: {cliente_sel or 'Todos'}")
        meta4.caption(f"Resp. tipo: {responsable_tipo_sel or '-'}")
        meta5.caption(f"Responsable: {responsable_sel or 'Todos'}")
        ...
        filters_expanded_cliente = bool(
            _normalize_focos_ui(st.session_state.get("applied_foco", []))
        ...
        with st.expander("FILTROS (opcional)", expanded=filters_expanded_cliente):
            with st.form("filters_form_cliente", clear_on_submit=False):
                f1, f2 = st.columns([1.3, 2.0], gap="small")
        ...
        def _render_rr_people_context():
            ...
            try:
                ...
                st.warning("No pude cargar el contexto RR para este scope.")
                return
            ...
            with st.expander(f"Contexto RR ({len(rr_view)} filas)", expanded=False):
        ...
        if scope_level == "L0":
            ...
            df_resp_view = _drop_total_rows(df_resp).rename(columns={
            ...
            df_cli_view = _drop_total_rows(df_cli).rename(columns={
                "cliente": "CLIENTE",
        ...
        # -----------------------------
            ...
            df_local_view = _drop_total_rows(df_local).rename(columns={
                "cod_rt": "COD_RT",
        ...
        st.caption("CLIENTE: export diferido hasta cerrar estabilidad de render y cero regresión.")
        st.stop()
    ...
    if modo == "CONTROL GESTION":
        ...
        def _cg_show(title: str, df: pd.DataFrame | None) -> None:
            st.markdown(f"#### {title}")
            ...
            if module_sel == "Inicio":
                if role_sel == "JEFE OPERACIONES":
                    with _timed("PAGE cg_inicio_jefe", tag="PAGE"):
                        df_inicio = db.get_cg_inicio_jefe(limit=200, offset=0)
                else:
                    with _timed("PAGE cg_inicio_gestor", tag="PAGE"):
                        df_inicio = db.get_cg_inicio_gestor(limit=200, offset=0)

                with _timed("PAGE cg_alertas_control_gestion", tag="PAGE"):
                    df_alertas = db.get_cg_alertas_control_gestion(limit=200, offset=0)

                _cg_show("Inicio", df_inicio)
                _cg_show("Alertas", df_alertas)

            else:
                with _timed("PAGE cg_cumplimiento_semana_local_parity", tag="PAGE"):
                    df_parity = db.get_cg_cumplimiento_semana_local_parity(limit=200, offset=0)
                with _timed("PAGE cg_cumplimiento_semana_scope", tag="PAGE"):
                    df_scope = db.get_cg_cumplimiento_semana_scope(limit=200, offset=0)
                with _timed("PAGE cg_cumplimiento_detalle", tag="PAGE"):
                    df_detalle = db.get_cg_cumplimiento_detalle(limit=200, offset=0)

                _cg_show("Cumplimiento parity", df_parity)
                _cg_show("Cumplimiento scope", df_scope)
                _cg_show("Detalle", df_detalle)
