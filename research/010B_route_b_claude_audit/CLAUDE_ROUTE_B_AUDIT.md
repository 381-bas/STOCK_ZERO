...
**Status:** FINAL â€” NO IMPLEMENTATION AUTHORIZED BY THIS FILE
...
- `kpione2_photo_export_contract_v1.json` â€” active contract (ACTIVE, do not modify)
- `load_control_gestion_raw_v17.py` â€” existing KPIONE2 raw loader (guarded)
- `cg_route_weekly_local_lab.py` â€” existing route lab (reference for lab patterns)
- `AGENT_AUTHORITY_MATRIX_V2.json` â€” active authority model
- `FAST_REFORM_010A_route_b_implementation_lock.json` â€” phase lock

---

## Q1: Does Route B preserve photo_row â†’ event_row â†’ day_presence?
...
- `Foto NÂº/Total` â€” photo sequence denominator, varies within an event
- `Link Foto` â€” per-photo URL, photo-level
- `Hora` â€” varies within event ID
- `Tipo de Tarea` â€” task label varies by photo within same visit event
- `Fecha de subida` â€” upload metadata, must not affect visit identity

Derivations are specified at the event level:
- `n_fotos_calculado = max(parse_total(Foto NÂº/Total)) per ID`
...
The chain is contractually correct. It has not been proven in code yet â€” that is 010B's job.
...
### Risk R1 â€” Direct row-to-presence mapping (CRITICAL)
...
The existing `load_kpione2` function in `load_control_gestion_raw_v17.py` (lines 712â€“733)
...
### Risk R2 â€” Incorrect aggregation key (MEDIUM)
If event aggregation groups by `(Codigo Local, Marca, Fecha)` only â€” without including `ID` â€”
...
### Risk R3 â€” Day presence from event count, not binary (LOW-MEDIUM)
...
### Risk R4 â€” Hash contamination from photo-level columns (LOW)
...
The contract explicitly excludes these â€” Codex must not add them back.

---

## Q3: Classification by authority level

### GREEN â€” Total autonomy (no approval needed, no cross-audit)
...
### YELLOW â€” Controlled autonomy (no approval, but gated by phase)
...
| Open PR | GitHub | For Claude + BastiÃ¡n review |

### ORANGE â€” Cross-audit required before PR is merge-ready

The following areas require **both** Claude and Codex sign-off before merge:

| Area | Files | Why ORANGE |
|---|---|---|
| Modifying existing KPIONE2 loader | `scripts/load_control_gestion_raw_v17.py` | Guarded loader â€” live compliance pipeline |
...
| Modifying `event_stable_hash_columns` | `kpione2_photo_export_contract_v1.json` | Active contract change â€” requires cross-audit |

### RED â€” BastiÃ¡n explicit authorization required
...
| Modifying `kpione2_photo_export_contract_v1.json` | Active contract â€” governance |
...
1. **Create** `scripts/load_kpione2_photo_from_excel.py`
   - Must implement `photo_row â†’ event_row â†’ day_presence` grouping
...
3. **Create** `sql/15_kpione2_photo_raw_ddl.sql` â€” DDL file ONLY, header MUST start with
...
### CA1 â€” Grain aggregation correctness (HIGHEST PRIORITY)
Codex must show that the `photo_row â†’ event_row` grouping is implemented as:
```python
...
The following are unconditionally blocked until BastiÃ¡n provides explicit written authorization
in the issue/PR for the specific action:

1. `DB apply` â€” any DDL or DML execution against Supabase real
2. `SQL apply` â€” execution of any `.sql` file against Supabase
...
explicit PR comment from BastiÃ¡n.
...
If any of E1â€“E7 is missing, the PR is not merge-ready.

---

## Q8: What rollback must exist?

Route B must be implemented **additively** â€” it must not modify or replace the existing
...
- `sql/16_kpione2_photo_raw_ddl_rollback.sql` â€” `DROP TABLE IF EXISTS cg_raw.kpione2_photo_raw`
- This file must also start with `-- NO APPLY`

### Integration rollback (for loader integration phase â€” ORANGE+)
...
- `scripts/load_kpione2_photo_from_excel.py` â€” new photo loader
- `sql/15_kpione2_photo_raw_ddl.sql` â€” DDL only, must have `-- NO APPLY` header
- `sql/16_kpione2_photo_raw_ddl_rollback.sql` â€” rollback DDL, must have `-- NO APPLY` header
- `tests/test_kpione2_photo_grain.py` â€” unit tests (or equivalent test path)
...
- `scripts/scanner.py` â€” ONLY if the scanner needs to discover the new photo export file pattern, AND only after Claude reviews the change
...
### Guarded loaders (ORANGE boundary â€” loader logic change)
- `scripts/load_control_gestion_raw_v17.py`
- `scripts/refresh_control_gestion_v2_incremental.py`
- `scripts/refresh_control_gestion_v2_mv.py`
...
### Active contract (RED â€” governance)
- `contracts/control_gestion/kpione2_photo_export_contract_v1.json`

### Governance artifacts (RED â€” governance layer)
...
All RED actions require BastiÃ¡n's explicit written authorization.*
